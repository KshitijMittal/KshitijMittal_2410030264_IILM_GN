...
SELECT * 						-- In Simple words 👉 “Employees who are not managers but have a manager as their boss”
   FROM employee
   WHERE 
      JOB <> "MANAGER"
      AND 
...
SELECT e.ENAME, m.ENAME AS Manager_Name, d.DNAME AS Department_Name,  
  m.DEPTNO AS Manager_Department_Number, e.DEPTNO AS Employee_Department_Number
  FROM employee e 
  JOIN department d 
    ON e.DEPTNO = d.DEPTNO
  JOIN employee m
